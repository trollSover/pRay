...
/*		const float3 vfDiffuseColor = (fAmbientLight + vfDiffuse) * tx_TextureColor;
		...
		//float3 textureColor = g_smiley.Sample(g_ssSampler, vfUV).xyz;
		//vfFinalColor = g_smiley.Sample(g_ssSampler, vfUV).rgba;
	...
	//float4 color = mul(float4(1.f, 1.f, 1.f, 1.f), g_mfWorld);
	//g_uResultTexture[DTid.xy] = color;
